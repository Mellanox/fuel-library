class mellanox_neutron_fuel::neutron_server_base{

    # Ensure openstack-neutron-mellanox is installed
    package{'openstack-neutron-mellanox': ensure=>installed}

    File_line{path => '/etc/neutron/neutron.conf', require => Package['openstack-neutron-mellanox']}
    $file_lines = {
        '/etc/neutron/neutron.conf'=>
           {line     => 'core_plugin = neutron.plugins.mlnx.mlnx_plugin.MellanoxEswitchPlugin',
             match   => "^core_plugin\s*="},
        'neutron.conf_mysql'=>
           {line    => "connection = mysql://neutron:${mlnx_neutron_root_password}@${controller_inband_IP}/neutron",
    ...
    file { 'mlnx_conf.ini':
      ...
      content => template('mellanox_neutron/mlnx_conf_template.erb'),
    }
    ...
    service{'neutron-server': ensure => stopped, enable => false, require => File['/etc/neutron/plugin.ini']}
}
